        ...
        <>
            <div className={cn("text-white flex-col gap-y-5", open ? "hidden" : "flex")}>
                ...
                <div className='w-full h-52 overflow-hidden sm:h-96 flex justify-center items-center lg:rounded-xl'
                    ...
                    <Image src={lista?.imageUrl!} alt='image' width={2000} height={300} className='object-fill ' />
                </div>
                <div className='p-5 flex flex-col gap-y-3 text-center sm:text-left'>
                    ...
                    <div className='flex flex-col gap-1'>
                        ...
                        <div className='flex sm:flex-row flex-col justify-between sm:items-center gap-y-2'>
                            ...
                            <span className='flex items-center justify-center sm:justify-start'><MapPin className='h-5 w-5' />{lista?.discoteca.indirizzo} {lista?.discoteca.civico}, {lista?.discoteca.city}</span>
                        </div>
                    </div>
                    <div>
                        ...
                        <div className='flex sm:flex-row flex-col justify-between sm:items-center'>
                            <span className='text-center'>{lista?.bigliettiInfiniti ? "Biglietti infiniti" : `Biglietti rimanenti: ${lista?.bigliettiRimanenti}`}</span>
                            <span className='text-center'>Costo biglietto singolo: {lista?.prezzoBiglietto}€ </span>
                            {lista?.dataLimite ? (
                                <span className='text-center'>Termina il: {format(new Date(lista.dataLimite), "MMMM do, yyyy")}</span>
                            ) : (
                ...
                <div className='w-[50%] m-2 mx-auto flex items-center justify-center transition cursor-pointer bg-black rounded-full py-3 text-center border border-white group'
                    onClick={onCheckout}>
