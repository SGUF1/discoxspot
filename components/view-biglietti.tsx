              ...
              <div className="flex w-[95%] sm:w-[95%]  mt-2 justify-between">
                <div className="flex flex-col gap-1" onClick={() => {}}>
                  ...
                  <div className="flex ">
                    <MapPin size={20} />
                    <span className="ml-1">
                      {item.lista.discoteca.indirizzo}{" "}
                      {item.lista.discoteca.civico}, {item.lista.discoteca.city}
                      , {item.lista.discoteca.provincia.name}
                    </span>
                  </div>
                </div>
                <div className="flex flex-col gap-1">
                  <span>
                    {format(new Date(item.lista.dataLimite), "MMMM do, yyyy")}
                  </span>
                  <span
                    className="text-right text-xl font-bold cursor-pointer"
                    onClick={() => {
                      changeOpen();
                      !item.confermato && setCodice(item.codice!);
                    }}
                  >
                    {item.confermato ? "Convalidato" : item.codice}
                  </span>
                  <span
                    className="flex justify-end cursor-pointer"
                    onClick={() => shareContent(item.listaId)}
                  >
