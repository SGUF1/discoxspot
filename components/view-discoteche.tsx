...
const ViewDiscoteche = ({ discoteche, province }: ViewDiscotecheProps) => {
    return (
        <div className='mt-10 grid grid-cols-1 overflow-y-scroll overflow-x-auto h-[70vh] sm:grid-cols-2 lg:grid-cols-3 w-[85%] gap-8 text-white'>
            {discoteche.map((item) => (
                <div className='flex flex-col items-center' key={item.id}>
                    <div className='h-36 sm:h-48 flex items-center w-56 sm:w-64 lg:w-80 overflow-hidden rounded-xl'>
                    ...
                    <div className='flex w-56 sm:w-64 lg:w-80 mt-2 justify-between'>
                        <div className='flex flex-col gap-1'>
                            <div>{item.name}</div>
                            <div>{item.indirizzo} {item.civico}, {item.cap}, {province?.find(provincia => provincia.id === item.provinciaId)?.name}</div>
                        </div>
                        <div className='flex items-center'><Heart className="h-5 w-5" /> <span className='ml-2'>{item.like}</span></div>
                    </div>
                </div>
            ))}
        </div>
